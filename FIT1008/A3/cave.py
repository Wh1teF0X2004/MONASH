""" Implement methods and attributes for Caves for mining.
...
CAVE_NAMES = [
    ...
    "Benkongerike",
    # A medium-sized cave southeast of Saering's Watch and northwest of Headwaters of Harstrad containing bristlebacks and rieklings.
    "Blackbone Isle Grotto",  # A small cave connected to the sea, situated on Blackbone Isle.
    "Bleakcoast Cave",
    # A small cave southeast of Winterhold and directly east of the Shrine of Azura occupied by frost trolls.
    "Blind Cliff Cave",
    # A medium-sized cave southwest of Karthwasten along the banks of the Karth River occupied by Forsworn.
    ...
    "Broken Oar Grotto",
    # A large cave connected to the sea, located north of Solitude and northwest of Brinewater Grotto.
    ...
    # A medium-sized cave on the north coast of Solstheim containing bristlebacks and rieklings.
    ...
    "Coldcinder Cave",
    # A small cave with two entrances: one via a grate in the Bulwark Jail, and the other through a trap door atop the cliff just southeast of Raven Rock, near the top of the Bulwark.
    ...
    "Crystaldrift Cave",
    # A small cave that serves as the resting place for an insane Bosmer, with a shrine Kynareth built to watch over him and to bring peace to weary travelers.
    "Darkfall Cave",  # A small cave just south-southwest of the Orc stronghold Mor Khazgur.
    "Darkfall Passage",
    # A small cave which is only accessible through Darkfall Cave, and is the only access to the Forgotten Vale.
    ...
    "Fallowstone Cave",
    # A small cave with an exit to Giant's Grove, located in the Velothi Mountains northeast of Riften and a short distance north of Lost Prospect Mine.
    "Forebears' Holdout",  # A small cave southeast of Dragon Bridge.
    "Forsaken Cave",  # A medium-sized cave located west of Windhelm.
    "Frossel",
    # A small cave on a peninsula east of Haknir's Shoal and north of Skaal Village containing bristlebacks and rieklings.
    "Frostmoon Crag",
    # A small camp under a rock overhang south of Mount Moesring and northeast of the Abandoned Lodge inhabited by werewolves.
    "Frostroot Cave",  # A cave in Eastmarch, north of Kagrenzel.
    "Glacial Cave",
    # A small cave north of Benkongerike, carved into the glacial cliff face along the northeastern coast, containing rieklings and horkers.
    "Glenmoril Coven",
    # A small cave far to the northwest of Falkreath inhabited by a group of unique hagravens, the Glenmoril Witches.
    ...
    "Greywater Grotto",
    # A small cave far east of Falkreath, located south-southwest of Helgen, containing leveled predatory animals.
    ...
    "Halldir's Cairn",
    # A small cave and tomb in the southernmost part of Skyrim that was built to house the remains of the fierce Halldir.
    "Hob's Fall Cave",
    # A small cave on the coast between Winterhold and Dawnstar inhabited by leveled warlocks and skeletons.
    ...
    "Liar's Retreat",
    # A small cave and connected underground cellar northwest of Broken Tower Redoubt and northeast of Karthwasten.
    ...
    "Moss Mother Cavern",
    # A small, open-roofed cavernous grove lush with many kinds of flora and fungi, located due north of Hunter's Rest and northwest of Half-Moon Mill.
    ...
    "Ravenscar Hollow",
    # A small cave inhabited by hagravens west of the Thalmor Embassy and due east of the Steed Stone.
    ...
    "Red Eagle Redoubt",
    # A small cave and two exterior camps, one small and one sprawling, all occupied by Forsworn located east of Markarth.
    ...
    # A small cave on a small remote icy island off the north coast of Skyrim, where Septimus Signus has made his home.
    ...
    "Tolvald's Cave",
    # A large cave in the Velothi Mountains, northeast of Shor's Stone and south-southeast of Ansilvund.
    "Uttering Hills Cave",  # A small cave in the mountains southwest of Windhelm occupied by bandits.
    "White River Watch",
    # A small cave east-northeast of Honningbrew Meadery and southwest of the Ritual Stone occupied by the White River bandits.
...
class Cave:
    """
    Creates cave of different materials and material quantity that can be mined

    Attributes:
         name = Used to identify the cave
         material = Specify the material stored within this cave, 1 cave 1 material
         quantity = amount of the material which is currently mineable in the cave

    Getters:
         get_cave_name = Return cave name
         get_material = Return material in cave
         get_quantity = Return material in cave

    Methods:
         add_quantity = Add the amount of the material which is currently mineable in the cave
         remove_quantity = Remove the amount of the material which is currently mineable in the cave
         random_cave = Generates a cave randomly
    """
    ...
    def add_quantity(self, amount: float) -> None:
        """ Add the amount of the material which is currently mineable in the cave, return None """
        self.quantity += amount

    def remove_quantity(self, amount: float) -> None:
        """ Remove the amount of the material which is currently mineable in the cave, return None """
        self.quantity -= amount

    def get_cave_name(self) -> str:
        """ Return cave name """
        return self.name

    def get_material(self) -> Material:
        """ Return material in cave """
        return self.material

    def get_quantity(self) -> float:
        """ Get the amount of the material which is currently mineable in the cave, return float """
        return self.quantity

    def __str__(self) -> str:
        """ String representation of cave and its quantity and material, return String """
        return f'<Cave: {self.get_cave_name()} {self.get_quantity()} {self.get_material()}>'

    @classmethod
    def random_cave(cls, material_list: list[Material]) -> Cave:
        """ Generate and return a random cave and its quantity and material """
        cave_name = RandomGen.random_choice(CAVE_NAMES)
        material = RandomGen.random_choice(material_list)
        quantity = RandomGen.randint(5, 10)
        return Cave(cave_name, material, quantity)
