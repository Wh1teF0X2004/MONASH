...
SELECT DISTINCT 
TO_CHAR(Transaction_Date, 'MonYYY') AS Time_ID,
...
SELECT TO_CHAR(Transaction_Date,'MonYYYY') AS Time_ID, Branch_ID, Book_ID, 
SUM(Quantity) AS Number_of_Books_Sold
FROM Book_Transaction
GROUP BY TO_CHAR(Transaction_Date,'MonYYYY'), Branch_ID, Book_ID ;
...
BEGIN 
    FOR ITEM IN Price_Cursor LOOP 
        IF Item.End_Date = 'Now' THEN -- if end date is 'Now', change to system date 
        ...
        SET Total_Sales = Number_Of_Books_Sold * Item.Price -- Item is the cursor 
